...
export function FormInput ({
  ...
  return (
    <label class={`${wrapperClass} relative flex flex-col gap-1 text-gray-700 dark:text-gray-300 group`}>
      ...
      <div class={`${className} ${type === 'password' ? '' : 'input-password'} flex items-center justify-between gap-2 w-full h-fit not-[[class*=text-]]:sm:text-sm rounded border-2 transition-colors border-gray-200 hover:bg-orange-50 focus-within:hover:bg-gray-100 dark:hover:bg-gray-700/50 dark:focus-within:hover:bg-gray-800 hover:border-orange-500/50 touch:group-active:border-orange-500/50 focus-within:bg-gray-100 dark:border-transparent dark:bg-gray-700/50 dark:outline-0 dark:hover:border-gray-600 dark:touch:active:border-gray-600 dark:focus-within:bg-gray-800 cursor-text`}>
        <input
          ref={inputRef}
          id={id}
          name={name}
